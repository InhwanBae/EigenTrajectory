...
class GPGraph(nn.Module):
    ...
    def forward(self, v_abs, v_rel):
        ...
        if self.include_original:
            ...
            v, _ = self.baseline_model(v, a) if self.weight_share else self.baseline_model[0](v, a)
            v_stack.append(v)
        ...
        if self.include_inter_group:
            ...
            a_e = generate_adjacency_matrix(v_e).squeeze(dim=0).detach()
            v_e, _ = self.baseline_model(v_e, a_e) if self.weight_share else self.baseline_model[1](v_e, a_e)
        ...
        if self.include_intra_group:
            ...
            a_i = generate_adjacency_matrix(v_i, mask=mask).squeeze(dim=0).detach()
            v_i, _ = self.baseline_model(v_i, a_i) if self.weight_share else self.baseline_model[2](v_i, a_i)
            v_stack.append(v_i)

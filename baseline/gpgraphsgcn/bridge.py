...
def model_forward_pre_hook(obs_data, obs_ori, addl_info=None):
    ...
    loc_pos = torch.arange(1, v_rel.size(2) + 1, device=v_rel.device, dtype=v_rel.dtype)
    loc_pos = loc_pos.unsqueeze(dim=-1).expand_as(v_rel)
    v_rel = torch.cat([loc_pos, v_rel], dim=1)  # concat loc_pos
